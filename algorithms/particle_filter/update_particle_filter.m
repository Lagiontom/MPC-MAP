function [particles, estimated_pose] = update_particle_filter(read_only_vars, public_vars)
    ...
    estimated_x = mean(particles(:,1));
    estimated_y = mean(particles(:,2));
    estimated_theta = atan2(mean(sin(particles(:,3))), mean(cos(particles(:,3))));
    
    estimated_pose = [estimated_x, estimated_y, estimated_theta];
